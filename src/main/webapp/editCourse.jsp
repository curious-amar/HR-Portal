...
<body>
	<%
	List<CourseModel> courseobj = (List<CourseModel>) session.getAttribute("sess1");
	%>

	<div class="container">
			<%
				for (CourseModel fm : courseobj)
				{
				%>
	
		<form action="courseUpdate01" class="form-group" style="width: 80%"
			...
			<div class="row">
				<div class="col-sm-4">
					...
					<input class="form-control size" name="courseId" type="text"
						id="crsID" placeholder="Course Name" readonly
						value=<%=fm.getCourse_id()%>>
					<p id="lettid"></p>
				</div>
				<div class="col-sm-4">
					...
					<input class="form-control size" name="courseName" type="text"
						...
						value=<%=fm.getCourse_name()%>>
					<p id="lettid"></p>
				</div>
				<div class="col-sm-4">

					<select name="courseDuration" class="form-select size" width="60%"
						id="coursedu"   required>
						 <option selected><%=fm.getDuration()%></option> 
			...
			<div class="row">
				<div class="col-sm-6">
					<input type="text" class="form-control size" maxlength=5
						name="courseFee" id="feeID" placeholder="Course fee" required value=<%=fm.getFee()%>>
					<p id="numid"></p>
				</div>
				<div class="col-sm-6">
					<input type="text" class="form-control size" name="location"
						id="locID" placeholder="Location" required value=<%=fm.getLocation()%>>
				</div>
			</div>
			<div class="row">
				<div class="col-sm-12">
					<button type="submit" class="btn btn-success"
						style="margin: 5% 0% 0% 0%; width: 20%">Submit</button>
					<p id="para1ht"></p>
				</div>
			</div>
		</form>
	...
	<script>
		...
		document.getElementById("crsID")
				.addEventListener("keypress", allowLett);
